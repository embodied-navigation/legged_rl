...
- 静止指令下无持续漂移或明显原地踏步；
- 训练和评估无 NaN 或崩溃，invalid 环境步比例不高于 0.1%；原始 invalid 计数及关节越限明细必须保留用于诊断。

当前验收允许极少量瞬时 soft-limit 越界；下一轮训练优化目标是 20 秒固定场景达到 100% episode 存活率，并继续降低 invalid rate，而不是放宽 URDF 硬限位。
