...
本轮改变机器人模型、默认站姿、初始高度和执行器 PD 参数，并增加不改变网络结构与优化器的数值稳定性保护。奖励、速度命令和环境随机化保持基线不变，以便与原训练结果有效对比。
...
H2 runner 增加两项数值稳定性保护：

- `clip_actions=1.0`：在动作进入环境和 `action_rate_l2` 计算前限制策略输出，避免异常动作让平方惩罚和 value target 无界增长；
- `noise_std_type="log"`：用 log standard deviation 参数化探索噪声，保证还原后的标准差始终为正。

这两项不改变动作维度、action scale、网络结构或 PPO 优化器参数。
